...
var main = function () {
  ...
  server.post('/api/download/audio/', (req, res) => {
    ...
    if (ytdlcore.validateURL(url)) {
      ...
      // Downloading file complete, converting to mp3
      // Process kind of takes a while, maybe could split up steps so client can get an idea of current progress
      audio.on('end', function () {
        try {
          ffmpeg(fs.createReadStream(filename))
            .audioBitrate(128)
            .save(`${filename}.mp3`)
            .on('end', () => {
              //fs.unlink(filename);
